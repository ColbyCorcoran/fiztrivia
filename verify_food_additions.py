#!/usr/bin/env python3
"""
Verify the 22 new Food questions don't duplicate existing questions
"""
...
def check_duplicates():
    ...
    # New question IDs (foo_279 to foo_300)
    new_question_ids = [f"foo_{i:03d}" for i in range(279, 301)]

    new_questions = [q for q in food_questions if q['id'] in new_question_ids]
    existing_questions = [q for q in food_questions if q['id'] not in new_question_ids]

    print("=" * 80)
    print("DUPLICATE CHECK FOR NEW FOOD QUESTIONS")
    print("=" * 80)
    ...
    if not found_issues:
        ...
        print("\nAll 22 new Food questions are unique.")
