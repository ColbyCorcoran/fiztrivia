#!/usr/bin/env python3
"""Verify new U.S. Geography questions for duplicates"""
import json
...
def check_duplicates():
    ...
    # Get the new U.S. Geography questions (geo_123 to geo_180)
    new_questions = [q for q in all_questions if q['id'] >= 'geo_123' and q['id'] <= 'geo_180']
    existing_questions = [q for q in all_questions if q['id'] < 'geo_123' or q['id'] > 'geo_180']

    print(f"Checking {len(new_questions)} new U.S. Geography questions against {len(existing_questions)} existing questions...\n")
