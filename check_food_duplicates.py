...
# Get all Food questions
food_questions = data['categories']['Food']

# Get newly added questions (foo_251 to foo_300)
new_question_ids = [f"foo_{i:03d}" for i in range(251, 301)]
new_questions = [q for q in food_questions if q['id'] in new_question_ids]

print(f"Checking {len(new_questions)} newly added Food questions against {len(food_questions) - len(new_questions)} existing questions...\n")

# Check for duplicates
duplicates_found = []

for new_q in new_questions:
    for existing_q in food_questions:
...
if duplicates_found:
    ...
    print("✅ No duplicates found! All new Food questions are unique.")
