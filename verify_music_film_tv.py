#!/usr/bin/env python3
"""Verify new Music in Film & TV questions for duplicates"""
import json
...
def check_duplicates():
    ...
    # Get the new Music in Film & TV questions (mus_022 to mus_060)
    new_questions = [q for q in all_questions if q['id'] >= 'mus_022' and q['id'] <= 'mus_060']
    existing_questions = [q for q in all_questions if q['id'] < 'mus_022' or q['id'] > 'mus_060']

    print(f"Checking {len(new_questions)} new Music in Film & TV questions against {len(existing_questions)} existing questions...\n")
