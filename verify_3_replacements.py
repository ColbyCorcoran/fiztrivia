#!/usr/bin/env python3
"""Verify the 3 replacement World Geography questions for duplicates"""
import json
...
def check_duplicates():
    ...
    # Get the new replacement questions (geo_301 to geo_303)
    new_questions = [q for q in all_questions if q['id'] >= 'geo_301' and q['id'] <= 'geo_303']
    existing_questions = [q for q in all_questions if q['id'] < 'geo_301' or q['id'] > 'geo_303']

    print(f"Checking {len(new_questions)} replacement questions against {len(existing_questions)} existing questions...\n")
    ...
    if duplicates_found:
        ...
        print("✅ No duplicates found! All replacement questions are unique.")
