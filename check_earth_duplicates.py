...
# Get all Earth questions
earth_questions = data['categories']['Earth']

# Get newly added questions (ear_251 to ear_300)
new_question_ids = [f"ear_{i:03d}" for i in range(251, 301)]
new_questions = [q for q in earth_questions if q['id'] in new_question_ids]

print(f"Checking {len(new_questions)} newly added Earth questions against {len(earth_questions) - len(new_questions)} existing questions...\n")

# Check for duplicates
duplicates_found = []

for new_q in new_questions:
    for existing_q in earth_questions:
...
if duplicates_found:
    ...
    print("✅ No duplicates found! All new Earth questions are unique.")
