#!/usr/bin/env python3
"""Verify new Landmarks & Monuments questions for duplicates"""
import json
...
def check_duplicates():
    ...
    # Get the new Landmarks & Monuments questions (geo_241 to geo_300)
    new_questions = [q for q in all_questions if q['id'] >= 'geo_241' and q['id'] <= 'geo_300']
    existing_questions = [q for q in all_questions if q['id'] < 'geo_241' or q['id'] > 'geo_300']

    print(f"Checking {len(new_questions)} new Landmarks & Monuments questions against {len(existing_questions)} existing questions...\n")
