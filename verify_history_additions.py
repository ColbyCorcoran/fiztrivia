#!/usr/bin/env python3
"""
Verify the 32 new History questions don't duplicate existing questions
"""
...
def check_duplicates():
    ...
    # New question IDs (his_269 to his_300)
    new_question_ids = [f"his_{i:03d}" for i in range(269, 301)]

    new_questions = [q for q in history_questions if q['id'] in new_question_ids]
    existing_questions = [q for q in history_questions if q['id'] not in new_question_ids]

    print("=" * 80)
    print("DUPLICATE CHECK FOR NEW HISTORY QUESTIONS")
    print("=" * 80)
    ...
    if not found_issues:
        ...
        print("\nAll 32 new History questions are unique.")
