#!/usr/bin/env python3
"""Verify new Maps & Borders questions for duplicates"""
import json
...
def check_duplicates():
    ...
    # Get the new Maps & Borders questions (geo_068 to geo_122)
    new_questions = [q for q in all_questions if q['id'] >= 'geo_068' and q['id'] <= 'geo_122']
    existing_questions = [q for q in all_questions if q['id'] < 'geo_068' or q['id'] > 'geo_122']

    print(f"Checking {len(new_questions)} new Maps & Borders questions against {len(existing_questions)} existing questions...\n")
