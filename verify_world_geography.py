#!/usr/bin/env python3
"""Verify new World Geography questions for duplicates"""
import json
...
def check_duplicates():
    ...
    # Get the new World Geography questions (geo_047 to geo_067)
    new_questions = [q for q in all_questions if q['id'] >= 'geo_047' and q['id'] <= 'geo_067']
    existing_questions = [q for q in all_questions if q['id'] < 'geo_047' or q['id'] > 'geo_067']

    print(f"Checking {len(new_questions)} new World Geography questions against {len(existing_questions)} existing questions...\n")
