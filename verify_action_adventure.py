#!/usr/bin/env python3
"""Verify Action/Adventure questions for duplicates"""
import json
...
def check_duplicates():
    ...
    # Get the new Action/Adventure questions (ent_052 to ent_106)
    new_questions = [q for q in all_questions if q['id'] >= 'ent_052' and q['id'] <= 'ent_106']
    existing_questions = [q for q in all_questions if q['id'] < 'ent_052' or q['id'] > 'ent_106']

    print(f"Checking {len(new_questions)} new Action/Adventure questions against {len(existing_questions)} existing questions...\n")
