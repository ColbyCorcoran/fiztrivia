...
# Get all Bible questions
bible_questions = data['categories']['Bible']

# Get newly added questions (bib_251 to bib_300)
new_question_ids = [f"bib_{i:03d}" for i in range(251, 301)]
new_questions = [q for q in bible_questions if q['id'] in new_question_ids]

print(f"Checking {len(new_questions)} newly added Bible questions against {len(bible_questions) - len(new_questions)} existing questions...\n")

# Check for duplicates
duplicates_found = []

for new_q in new_questions:
    for existing_q in bible_questions:
...
if duplicates_found:
    ...
    print("✅ No duplicates found! All new Bible questions are unique.")
